...
class GaussianExtractor:
    def __init__(self, scene, render, pipe, bg_color=None):
        """
    ...
    def reconstruction(self, viewpoint_stack):
        ...
        for i, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="reconstruct radiance fields"):
            render_pkg = self.render(viewpoint_cam, self.scene)
            rgb = render_pkg["render"]
            alpha = render_pkg["alpha"]
            normal = torch.nn.functional.normalize(render_pkg["normal"], dim=0)
            depth = render_pkg["depth"] * (alpha > 0.95)
            depth_normal = render_pkg["normal_ref"]
    ...
    def export_image(self, path):
        ...
        for idx, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="export images"):
            ...
            save_img_f32(
                self.depthmaps[idx][0].cpu().numpy(), os.path.join(vis_path, "depth_{0:05d}".format(idx) + ".tiff")
            )
            save_img_u8(
