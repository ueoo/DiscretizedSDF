...
class Camera(nn.Module):
    def __init__(
        ...
        if normal_image is not None:
            ...
            original_normal_image = self.original_normal_image * 2 - 1.0  # (0, 1) -> (-1, 1)
            original_normal_image = torch.nn.functional.normalize(original_normal_image, p=2, dim=0)
            fg_mask = torch.where((self.original_normal_image == 0.0).sum(0) < 3, True, False)[None, ...].repeat(
                3, 1, 1
            )
        ...
        if False:
            ...
            if self.original_normal_image is not None:
                self.original_normal_image *= torch.ones(
