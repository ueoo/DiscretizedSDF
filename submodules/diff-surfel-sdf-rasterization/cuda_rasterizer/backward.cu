	...
	float* __restrict__ dL_dpbrs)
{
	...
	__shared__ float collected_pbr[PBR_LEN * BLOCK_SIZE];
	...
	if (inside) {
		...
		for (int i = 0; i < PBR_LEN; i++)
            dL_dpbr[i] = dL_depths[(PBR_OFFSET+i) * H * W + pix_id];
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			for (int i = 0; i < PBR_LEN; i++)
				collected_pbr[i * BLOCK_SIZE + block.thread_rank()] = pbr_params[coll_id * PBR_LEN + i];
		}
		block.sync();
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
            ...
            {
                const float pbr_param = collected_pbr[ch * BLOCK_SIZE + j];
                accum_rec_pbr[ch] = last_alpha * last_pbr[ch] + (1.f - last_alpha) * accum_rec_pbr[ch];
                last_pbr[ch] = pbr_param;
                const float dL_dchannel_pbr = dL_dpbr[ch];
				dL_dalpha += (pbr_param - accum_rec_pbr[ch]) * dL_dchannel_pbr;
                atomicAdd(&(dL_dpbrs[global_id * PBR_LEN + ch]), dchannel_dcolor * dL_dchannel_pbr);
            }
